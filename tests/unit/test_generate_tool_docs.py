...
def test_generate_pages_writes_utf8(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    """Generated documentation must not depend on the system code page."""
    output_path = tmp_path / "tools.mdx"
    written_encodings: list[str | None] = []
    original_write_text = Path.write_text

    def record_write_encoding(
        path: Path, data: str, *args: object, **kwargs: object
    ) -> int:
        written_encodings.append(kwargs.get("encoding") if kwargs else None)
        return original_write_text(path, data, *args, **kwargs)

    monkeypatch.setattr(
        generator,
        "render_pages",
        lambda *_args: {output_path: "Unicode: \U0001f4dd"},
    )
    monkeypatch.setattr(Path, "write_text", record_write_encoding)

    generator.generate_pages(
        {},
        {},
        ToolCounts(0, 0, 0, 0, 0, 0, 0, 0),
        tmp_path,
        tmp_path,
        output_path,
    )
    ...
    assert output_path.read_text(encoding="utf-8") == "Unicode: \U0001f4dd"


def test_load_overrides_reads_utf8(tmp_path: Path) -> None:
    """Unicode guidance in overrides must survive Windows code pages."""
    override_path = tmp_path / "jira_example.yaml"
    override_path.write_text('tips: "Unicode: \u2014"\n', encoding="utf-8")

    overrides = load_overrides(tmp_path)

    assert overrides["jira_example"].tips == "Unicode: \u2014"
